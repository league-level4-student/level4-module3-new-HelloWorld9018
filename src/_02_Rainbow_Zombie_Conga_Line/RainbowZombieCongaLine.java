...
public class RainbowZombieCongaLine {
    ...
    public void engine(Zombie dancer) {
    	...
    	while(zom.getValue() != dancer) {
    		zom = zom.getNext();
    	}
    	congaLine.setHead(zom);*/
    }
    ...
    public void everyoneOut(Zombie dancer) {
    	...
    	while(zom != null) {
    		position += 1;
    	if(zom.getValue().getZombieHatColor().equals(dancer.getZombieHatColor())) {
    ...
    public void rainbowBrains(Zombie dancer) {
    	...
    	for(int i = 0; i<zombieHats.length; i++) {
    		Zombie dancerZom = (new Zombie(zombieHats[i]));
    		congaLine.add(dancerZom);
    	}
    }
